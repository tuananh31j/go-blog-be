...
func CreateProject(apctx appctx.AppContext) func(c *fiber.Ctx) error {
	return func(c *fiber.Ctx) error {
		...
		biz := blogBusiness.NewCreateProjectBiz(serviceBlog)
		err = biz.CreateProject(c.Context(), &payload)
		if err != nil {
			logger.Err(err).Msg("Failed to create project")
			panic(err)
		}
		return c.Status(fiber.StatusCreated).JSON(common.SimpleSuccessResponse("Project created successfully"))
	}
}
